...
let translate_program (p : Input.typed_program) : Output.program =
  ...
  let offset_of_field (class_def : Input.typed_class) field :
    ...
    let rec calc fields =
      match fields with
      | (name, t) :: sub ->
          if name = field then 0 else sizeof t + calc sub
      | [] ->
          raise
            (ImplementationError
               (Printf.sprintf
  ...
  and tr_mem (mem_access : Input.typ Input.mem) : Input.typed_expression =
    ...
    | Atr (id, field) -> (
        ...
        | TClass name ->
            ...
            {expr= Binop (Add, id, offset); annot= TInt}
        | _ ->
            raise (ImplementationError "not a class when computing offset") )
  in
  ...
  in
  {Imp.globals= List.map fst p.globals; functions= List.map tr_fdef p.functions}
